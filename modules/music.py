...
detailed_help = {
    ...
    "Subcommands": f"`{client.default_prefix}music join` - Joins a voice channel\n"
                   ...
                   f"`{client.default_prefix}music info [index]` - Shows song information for the specified song in "
                   f"the queue\n "
...
async def command(command: str, message: discord.Message):
    ...
    if not voice_enable:
        await message.channel.send(
            "Sorry, but the internal Opus library required for voice support was not loaded for whatever reason, and music functionality will not work.")
        return
            ...
            try:
                ...
                if target_song is None:
                    ...
                    await message.channel.send(embed=get_song_embed(target_song, is_next=True,
                                                                    is_paused=get_target_voice_connection(
                                                                        message.guild).is_paused()))
                    return
