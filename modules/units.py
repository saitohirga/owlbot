...
help_dict = {
    ...
    "Description": "This units transforms one unit into another unit. The command uses the underlying `units` Unix "
                   "command, and supports over 3000 different units.\nIf destination units are not provided, "
                   "the source units will be converted into standard SI units.",
    ...
    "Two argument usage": "`units 41.28mi km`\n`units 12um 0.02mm`\nWhen passing two arguments, the format is `units "
                          "<src> <dst>`. This will convert the given measurement into the destination unit, "
                          "if possible. If two numerical measurements are provided, this performs a ratio comparison "
    ...
    "Using temperature": "If it is desired to use temperatures, the temperature functions must be used: `tempC("
                         "1700)`, `tempK(273.15)`, `tempF`, or `tempR`.",
    ...
    "Comparing measurements": "It is possible to pass in two whole measurements in order to compare the two, assuming "
                              ...
                              "eg. `units 5km 2.5km` returns 2.0, because 5km is twice as large as 2.5km). Using units "
                              "that cannot be compared results in a conformability error.",
